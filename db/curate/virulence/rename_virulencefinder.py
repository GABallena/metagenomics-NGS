...
# Define paths
input_dir = Path("virulencefinder_db")
output_fasta = Path("virulencefinder_db_renamed.fasta")
output_metadata = Path("virulencefinder_db_metadata.tsv")
...
for fasta_file in sorted(input_dir.glob("*.fsa")):
    for record in SeqIO.parse(fasta_file, "fasta"):
        ...
        new_id = f"VirulenceFinder_VirulenceGene{gene_counter}"
        gene_counter += 1
